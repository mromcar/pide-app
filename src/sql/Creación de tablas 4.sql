...
DROP VIEW IF EXISTS menu_translated CASCADE; -- CASCADE para asegurar eliminación de dependencias de la vista

DROP TABLE IF EXISTS product_variant_history CASCADE;
DROP TABLE IF EXISTS product_history CASCADE;
DROP TABLE IF EXISTS order_status_history CASCADE;
DROP TABLE IF EXISTS order_items CASCADE;
-- DROP TABLE IF EXISTS order_detail CASCADE; -- ELIMINADO: Esta tabla no existe en tu esquema
...
CREATE TABLE products (
  ...
  description TEXT,           -- Descripción en idioma por defecto
  image_url VARCHAR(255),
  sort_order INT DEFAULT 0,
  is_active BOOLEAN DEFAULT TRUE,
...
-- Tabla de historial para productos (sin precio, ya que está en variantes; descripción y nombre de la tabla principal o de la traducción por defecto)
...
-- Tabla de historial para variantes de producto (cambiado de price_delta a price)
CREATE TABLE product_variant_history (
  id SERIAL PRIMARY KEY,
  variant_id INTEGER REFERENCES product_variants(variant_id) ON DELETE CASCADE,
  variant_description TEXT, -- Descripción de la variante en el momento del cambio (ej. la por defecto)
  price DECIMAL(10,2), -- Precio en el momento del cambio
  is_active BOOLEAN,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
-- ===== PASO 4: Vistas =====
-- VISTA AJUSTADA: Eliminado p.price y ajustado join para descripciones si es necesario de product_translations
-- Si la idea es mostrar solo la descripción principal (ej. en español 'es')
...
SELECT
  ...
  pt.description AS product_description, -- Renombrado para claridad
  p.is_active,
  pt.language_code,
  pv.variant_id,
  pvt.variant_description, -- Descripción de la variante traducida
  pv.price AS variant_price -- Precio de la variante
FROM categories c
JOIN category_translations ct ON ct.category_id = c.category_id
JOIN products p ON p.category_id = c.category_id
JOIN product_translations pt ON pt.product_id = p.product_id AND pt.language_code = ct.language_code -- Unir por idioma también
LEFT JOIN product_variants pv ON pv.product_id = p.product_id AND pv.is_active = TRUE AND pv.deleted_at IS NULL -- Unir variantes activas
LEFT JOIN product_variant_translations pvt ON pvt.variant_id = pv.variant_id AND pvt.language_code = pt.language_code -- Unir traducciones de variantes
WHERE c.deleted_at IS NULL
  AND p.deleted_at IS NULL
  AND c.is_active = TRUE -- Añadir is_active para categorías
...
-- historial producto (AJUSTADO: ya no obtiene precio de products)
CREATE OR REPLACE FUNCTION archive_product()
RETURNS TRIGGER AS $$
BEGIN
  -- Historiar el nombre y descripción principales (por defecto en la tabla products)
...
AFTER UPDATE ON products -- AFTER UPDATE es más común para historial
FOR EACH ROW
...
-- historial variante (AJUSTADO: obtiene descripción de la variante y precio actual)
...
AFTER UPDATE ON product_variants -- AFTER UPDATE para historial
FOR EACH ROW
...
CREATE INDEX idx_categories_is_active ON categories(is_active); -- Nuevo índice para categorías
