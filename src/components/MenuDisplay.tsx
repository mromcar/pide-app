...
export default function MenuDisplay({ menu, lang }: MenuDisplayProps) {
  ...
  if (!menu || menu.length === 0) {
    return <p>{t.menuNoItems}</p>; // Usar la clave estandarizada
  }
      ...
      {menu.map((category) => (
        <section key={category.category_id} className="mb-12">
          ...
          {category.products.length === 0 ? (
            <p className="text-gray-500 italic ml-4">{t.menuNoProductsInCategory}</p> // Usar la clave estandarizada
          ) : (
                    ...
                    {product.allergens && product.allergens.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs text-gray-400">
                          {t.productAllergens}: {product.allergens.map(a => a.allergen.name).join(', ')}
                        </p>
                      </div>
                    )}
                  </div>
